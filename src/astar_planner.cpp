...
namespace  astar_planner{
    ...
    // FIX: O(1) Instant lookup array to prevent freezing
    ...
    while (!pending_nodes.empty() && rclcpp::ok() && iterations < max_iterations){
        ...
        for (const auto &dir : explore_dir) {
            ...
            if (!visited_nodes[new_node_idx] && costmap_->getCost(new_node.x, new_node.y) < 128){
                ...
                visited_nodes[new_node_idx] = true; // Mark as visited immediately
    ...
    if (path.poses.size() < 3) {
        RCLCPP_WARN(node_->get_logger(), "Path is too short to smooth. Returning raw path.");
        return path; 
    }
